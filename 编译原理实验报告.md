...
1.  **理论实践结合**：将抽象的上下文无关文法（CFG）转换为具体的程序逻辑，理解移进-归约（Shift-Reduce）冲突的本质及其解决方法。
2.  **LR(1) 构造过程掌握**：掌握从文法到 LR(1) 项目集规范族（Canonical Collection of Sets of Items），再到 DFA（确定有限自动机），最后生成 Action 和 Goto 表的完整流程。
3.  **语法树构建**：在分析过程中同步构建抽象语法树（AST），理解语义动作（Semantic Actions）如何在归约步骤中执行。
4.  **调试与错误处理**：通过设计针对特定复杂算术表达式（含浮点数、括号嵌套、连续运算）的测试用例，验证分析器的健壮性。
...
*   **核心功能**：
    1. **词法分析器 (Lexer)**：将原始字符流分割为 Tokens (如 `ID:x`, `ASSIGN:=`, `NUM:0.5` 等)。
    
    2. **LR(1) 总控程序**：维护一个状态栈和符号栈，根据当前状态和向前看符号（Lookahead）查表执行动作。
    
    3. **归约输出**：程序运行结束时，输出完整的产生式归约序列，证明输入串符合文法结构。
...
首先定义拓广文法，并构建项目集。每个项目形如 [A→α⋅β,a]，其中 a 是搜索符（Lookahead）。

**初始状态 `I0`:**

- `[S′→⋅assign_stmt,$]`

- 由于 `⋅` 后面是 `assign_stmt`，进行 **Closure (闭包)** 操作展开：

- [*a**ss**i**g**n*_s**t**m**t*→⋅*I**D*=*e**x**p*,$]

**状态转移 (Goto):**

- 在 `I0` 遇到 ID，转移到 `I1`: `[assign_stmt→ID⋅=exp,$]`
- 在 `I1` 遇到 =，转移到 `I2`: `[assign_stmt→ID=⋅exp,$]`
- 此时在 ，需要展`I2`开 exp 的所有产生式，Lookahead 为 $ 或 ; 等

### ![image-20251231114503063](C:\Users\alexa\AppData\Roaming\Typora\typora-user-images\image-20251231114503063.png)
...
2.  **符号的含义**：
    ...
    *   可以看到在 `I_Paren` 状态中，Lookahead 变成了 `)`，这清晰地展示了 LR(1) 如何根据上下文区分相同的产生式（例如区分 `y+10` 是在括号内还是在赋值语句末尾）。
3.  **边的含义**：
...
![image-20251231112537185](C:\Users\alexa\AppData\Roaming\Typora\typora-user-images\image-20251231112537185.png)
...
![image-20251231115547635](C:\Users\alexa\AppData\Roaming\Typora\typora-user-images\image-20251231115547635.png)
...
*   **状态含义**:
    ...
    *   **State 0**: 初始状态，期待程序开始。包含项目 $[S' \to \cdot assign\_stmt, \$]$。
...
**算法逻辑：**
1.  **确定产生式**：根据传入的规则号 `rule`，确定父节点的类型（如 `OpK` 或 `StmtK`）和子节点的数量（Pop Count）。
2.  **节点生成**：
...
4.  **状态转移**：弹出旧状态，查看当前栈顶状态，根据归约出的非终结符（如 `EXP`），查 `Goto` 表决定压入的新状态。
...
![image-20251231120556993](C:\Users\alexa\AppData\Roaming\Typora\typora-user-images\image-20251231120556993.png)
...
![image-20251231113948012](C:\Users\alexa\AppData\Roaming\Typora\typora-user-images\image-20251231113948012.png)
...
![image-20251231114107057](C:\Users\alexa\AppData\Roaming\Typora\typora-user-images\image-20251231114107057.png)
...
1.  **State 107/108 的归约缺失（连续加法问题）**
    ...
    *   **解决**：在 `getAction` 中为状态 107 和 108 增加了对 `PLUS` Token 的处理，返回对应的负值（Reduce操作）。
